...
%
% modified Dec 2014 to fix a bug in the file saving.
% Dec 2014 -- Josh McDermott <jhm@mit.edu>
%
% modified March 2017 to replace wavread/wavwrite with audioread/audiowrite
% Mar 2017 -- Josh McDermott <jhm@mit.edu>
...
elseif P.initialize_with_sound_file == 1
     [temp,sr] = audioread([P.initial_sound_folder P.initial_sound_filename]);
     if size(temp,2)==2
...
for it_n=1:P.num_it
    ...
    if ismember(it_n,P.iteration_snapshots)
        audiowrite([P.output_folder new_filename '_it' num2str(it_n) '.wav'], synth_sound, P.audio_sr);
    end
    ...
    synth_subbands = generate_subbands(synth_sound, audio_filts); % sig_len x num_subs
    synth_analytic_subbands = hilbert(synth_subbands);
    synth_subband_envs = abs(synth_analytic_subbands); % sig_len x num_subs
    ...
    synth_subband_envs = resample(synth_subband_envs,1,ds_factor); % sig_len/ds_factor x num_subs
...
if P.write_spec_match_noise
    ...
    audiowrite([P.output_folder 'sm_noise_' P.orig_sound_filename], noise, P.audio_sr);
end
...
if P.write_norm_orig
    if P.avg_stat_option==0
        if length(orig_sound)>1.5*length(synth_sound)
            audiowrite([P.output_folder 'norm_' P.orig_sound_filename], orig_sound(1:length(synth_sound)), P.audio_sr);
        else
            audiowrite([P.output_folder 'norm_' P.orig_sound_filename], orig_sound, P.audio_sr);
        end
    elseif P.avg_stat_option>=1 %for comparison, average of waveforms
        audiowrite([P.output_folder 'wvfm_avg_' P.avg_filename], waveform_avg, P.audio_sr);
    end
end

%save synthetic result
if P.imposition_windowing==2 %imposition is not circular, so leave off ends where window faded to zero
    audiowrite([P.output_folder new_filename '.wav'], synth_sound(P.audio_sr:end-P.audio_sr), P.audio_sr);
else
    audiowrite([P.output_folder new_filename '.wav'], synth_sound, P.audio_sr);
    if P.write_repeat
        audiowrite([P.output_folder new_filename '_rep.wav'], [synth_sound;synth_sound], P.audio_sr);
    end
end
