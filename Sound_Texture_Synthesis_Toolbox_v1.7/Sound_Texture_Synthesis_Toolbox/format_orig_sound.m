...
%
% modified March 2017 to replace wavread with audioread
% Mar 2017 -- Josh McDermott <jhm@mit.edu>

if P.avg_stat_option==0
    [temp,sr] = audioread([P.orig_sound_folder P.orig_sound_filename]);
%     warning('forcing the sound to 1 second');
%     temp = temp(1:sr);
elseif P.avg_stat_option>=1
    [temp,sr] = audioread([P.orig_sound_folder P.files_for_stat_avg{file_n}]);
end
if size(temp,2)==2
